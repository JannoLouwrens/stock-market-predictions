...
def get_flair_sentiment_score(text):
    if isinstance(text, str):  # Check if text is a string
        ...
        if sentence.labels:
            ...
            positive_prob = sentence.labels[0].score

            # Check if there is a negative label
            negative_prob = sentence.labels[1].score if len(sentence.labels) > 1 else 0.0
...
# Apply the functions to each row in the financial news data
merged_data['flair_sentiment_score'] = merged_data['text'].apply(get_flair_sentiment_score)
...
merged_data['lag_flair_sentiment_score_1'] = merged_data['flair_sentiment_score'].shift(1)
merged_data['lag_textblob_sentiment_score_1'] = merged_data['textblob_sentiment_score'].shift(1)
merged_data['lag_vader_sentiment_score_1'] = merged_data['vader_sentiment_score'].shift(1)
...
feature_set = pd.DataFrame({
    ...
    'sentiment_flair_score': merged_data['flair_sentiment_score'],
    ...
    'lag_flair_sentiment_score_1': merged_data['lag_flair_sentiment_score_1'],
    'lag_textblob_sentiment_score_1': merged_data['lag_textblob_sentiment_score_1'],
    'lag_vader_sentiment_score_1': merged_data['lag_vader_sentiment_score_1']
})

# Split the feature set into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(
    feature_set.drop(['date', 'closing_price',
...
actual_vs_predicted_with_data = pd.DataFrame({
    ...
    'Sentiment Flair Score': merged_data.loc[X_test.index, 'flair_sentiment_score'],
...
plt.plot(actual_vs_predicted_with_data['Date'], actual_vs_predicted_with_data['Actual Closing Price'],
         label='Actual Test Closing Price', marker='o')

# Add predictions for the test set to the same graph
plt.scatter(actual_vs_predicted_with_data['Date'], actual_vs_predicted_with_data['Predicted Closing Price'],
...
result_df_train = pd.DataFrame({
    ...
    'Sentiment Flair Score': merged_data.loc[X_train.index, 'flair_sentiment_score'],
